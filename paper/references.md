# References

> Cited as (Author, year) throughout [`draft.md`](draft.md). Entries for sections not
> yet drafted (§5–§9) are already listed, marked ◦.

- Anderson, P. W. (1972). *More Is Different.* Science, 177(4047), 393–396.
- Biderman, S., Schoelkopf, H., et al. (2023). *Pythia: A Suite for Analyzing Large
  Language Models Across Training and Scaling.* ICML 2023.
- Elhage, N., Nanda, N., Olsson, C., et al. (2021). *A Mathematical Framework for
  Transformer Circuits.* Transformer Circuits Thread.
- Hafner, D., Lillicrap, T., et al. (2019). *Learning Latent Dynamics for Planning from
  Pixels.* ICML 2019. (The RSSM architecture.)
- Haken, H. (1983). *Synergetics: An Introduction.* Springer. (Slow modes and the
  enslaving principle.)
- Hendel, R., Geva, M., & Globerson, A. (2023). *In-Context Learning Creates Task
  Vectors.* Findings of EMNLP 2023.
- Hoogland, J., Wang, G., et al. (2024). *The Developmental Landscape of In-Context
  Learning.* arXiv:2402.02364. (Developmental interpretability; LLC over training.)
- ◦ Kallini, J., Papadimitriou, I., Futrell, R., Mahowald, K., & Potts, C. (2024).
  *Mission: Impossible Language Models.* ACL 2024.
- Lau, E., Furman, Z., Wang, G., Murfet, D., & Wei, S. (2024). *The Local Learning
  Coefficient: A Singularity-Aware Complexity Measure.* arXiv:2308.12108. (The LLC
  estimator.)
- Noether, E. (1918). *Invariante Variationsprobleme.* Nachrichten von der Gesellschaft
  der Wissenschaften zu Göttingen, 235–257.
- Olsson, C., Elhage, N., Nanda, N., et al. (2022). *In-context Learning and Induction
  Heads.* Transformer Circuits Thread.
- Park, K., Choe, Y. J., & Veitch, V. (2023). *The Linear Representation Hypothesis and
  the Geometry of Large Language Models.* arXiv:2311.03658.
- OLMo Team (2024). *2 OLMo 2 Furious.* arXiv:2501.00656. (OLMo 2; fully open models,
  data, and checkpoints.)
- Qwen Team (2024). *Qwen2.5 Technical Report.* arXiv:2412.15115.
- Radford, A., Wu, J., et al. (2019). *Language Models are Unsupervised Multitask
  Learners.* OpenAI. (GPT-2.)
- Schaeffer, R., Miranda, B., & Koyejo, S. (2023). *Are Emergent Abilities of Large
  Language Models a Mirage?* NeurIPS 2023.
- Shai, A., Riechers, P., Teixeira, L., Oldenziel, A., & Marzen, S. (2024).
  *Transformers Represent Belief State Geometry in their Residual Stream.* NeurIPS 2024.
- Todd, E., Li, M., Sharma, A., Mueller, A., Wallace, B., & Bau, D. (2024). *Function
  Vectors in Large Language Models.* ICLR 2024.
- Wang, K., Variengien, A., Conmy, A., Shlegeris, B., & Steinhardt, J. (2023).
  *Interpretability in the Wild: a Circuit for Indirect Object Identification in GPT-2
  small.* ICLR 2023. (Mean ablation as standard practice.)
- Watanabe, S. (2009). *Algebraic Geometry and Statistical Learning Theory.* Cambridge
  University Press. (Singular learning theory; the RLCT.)
- Wei, J., Tay, Y., et al. (2022). *Emergent Abilities of Large Language Models.* TMLR.
- Zhang, F., & Nanda, N. (2024). *Towards Best Practices of Activation Patching in
  Language Models: Metrics and Methods.* ICLR 2024.
